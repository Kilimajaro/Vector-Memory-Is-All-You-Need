...
class GradioDialogSystem:
    ...
    def generate_response_stream(self, prompt, context, progress=gr.Progress()):
        ...
        system_prompt = """你是一个具有长期记忆能力的AI助手。在回答用户问题时，请遵循以下规则：

1. 首先分析检索到的记忆内容，这些记忆是系统从知识库中匹配的相关信息
2. 将检索结果与用户当前问题进行结合分析
3. 如果检索结果相关，请基于这些知识进行回答
4. 如果检索结果不相关，请基于你的通用知识回答，并说明没有找到相关记忆
5. 回答要自然、连贯，不要直接罗列检索结果
6. 在回答中适当引用检索到的关键信息
